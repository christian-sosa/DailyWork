...
def get_sfn_data():
    ...
    arn_list = {"ARN": "NOMBRE-SFN", "ARN": "NOMBRE-SFN"}
...
def get_lambda_metrics():
    ...
    lambda_names = {"ARN": "NOMBRE-LAMBDA", "ARN": "NOMBRE-LAMBDA"}
    metrics = []
...
def get_body(sfn_table: str, lambda_table: str) -> str:
    tTemplate = f"""
        <html>
            <head>
...
def lambda_handler(event, context):
    ...
    body = get_body(
        sfn_table=sfn_table.to_html(index=False, justify="center"),
        lambda_table=lambda_table.to_html(index=False, justify="center"),
    )
    title = f"Ejecuciones día { NOW.strftime('%d-%m-%Y') }"
    send_email(
        bcc_addresses=[],
        to_addresses=["QUIEN RECIVE@MAIL"],
        cc_addresses=[],
        body=body,
        sender="EL QUE ENVIA",
        title=title,
